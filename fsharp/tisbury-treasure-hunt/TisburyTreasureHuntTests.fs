...
let ``Get coordinate for Brass Spyglass`` () =
    getCoordinate ("Brass Spyglass", "4B")
    |> should equal "4B"

[<Fact>]
[<Task(1)>]
let ``Get coordinate for Robot Parrot`` () =
    getCoordinate ("Robot Parrot", "1C")
    |> should equal "1C"

[<Fact>]
[<Task(1)>]
let ``Get coordinate for Glass Starfish`` () =
    getCoordinate ("Glass Starfish", "6D")
    |> should equal "6D"

[<Fact>]
[<Task(1)>]
let ``Get coordinate for Crystal Crab`` () =
    getCoordinate ("Crystal Crab", "6A")
    |> should equal "6A"

[<Fact>]
[<Task(1)>]
let ``Get coordinate for Angry Monkey Figurine`` () =
    getCoordinate ("Angry Monkey Figurine", "5B")
    |> should equal "5B"
...
let ``Compare records for first matched records returns true`` () =
    ...
    compareRecords azarasData ruisData
    |> should equal true
...
let ``Create Record for first matched records returns correct tuple`` () =
    ...
    createRecord azarasData ruisData
...
let ``Compare records for third matched records returns correct tuple`` () =
    ...
    let expected =
        ("7E", "Quiet Inlet (Island of Mystery)", "Orange", "Vintage Pirate Hat")
