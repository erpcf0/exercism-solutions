...
[<Fact>]
[<Task(2)>]
let ``Price for pizza margherita with extra sauce`` () =
    pizzaPrice (ExtraSauce Margherita)
    |> should equal 8

[<Fact>]
[<Task(2)>]
let ``Price for pizza caprese with extra toppings`` () =
    pizzaPrice (ExtraToppings Caprese)
    |> should equal 11

[<Fact>]
[<Task(2)>]
let ``Price for pizza formaggio with extra sauce and toppings`` () =
    pizzaPrice (ExtraSauce(ExtraToppings Caprese))
    |> should equal 12

[<Fact>]
[<Task(2)>]
let ``Price for pizza caprese with extra sauce and toppings`` () =
    pizzaPrice (ExtraToppings(ExtraSauce Formaggio))
    |> should equal 13
...
[<Fact>]
[<Task(3)>]
let ``Order price for gigantic order`` () =
    orderPrice (List.replicate 100_000 Margherita)
    |> should equal 700_000
